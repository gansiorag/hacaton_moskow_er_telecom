...
def init_drive(path,brauser, vision = False):
    """
    Инициализация драйвера
    :param brauser: Название браузеров GoogleChrome Firefox
    :return:
    """
    full_path_chrom=os.getcwd().split(path)[0] + path + 'chromedriver/chromedriver'
...
def init_drivers(path = '/drivers/', brauser = 'GoogleChrome', kol = 1, vis = False):
    drivers=[]
    for i in range(kol):
...
if __name__ == "__main__":
    driver = init_drive('parser_AO_ATC', 'GoogleChrome')
    driver.get('https://www.google.com')
    print("Without visiual")
    driver.close
    driver = init_drive('parser_AO_ATC', 'GoogleChrome', True)
    driver.get('https://www.google.com')
